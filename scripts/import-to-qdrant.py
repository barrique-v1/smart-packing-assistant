...
def delete_collection(qdrant_url: str, collection_name: str):
    ...
    try:
        ...
        print(f"⚠️  Warning: Failed to delete collection: {e}")
...
def upload_points(qdrant_url: str, collection_name: str, points: List[Dict[str, Any]]):
        ...
        for attempt in range(RETRY_ATTEMPTS):
            try:
                ...
                if attempt < RETRY_ATTEMPTS - 1:
                    print(f"\n⚠️  Upload error (attempt {attempt + 1}/{RETRY_ATTEMPTS}): {e}")
                    print(f"   Retrying in {RETRY_DELAY} seconds...")
                    time.sleep(RETRY_DELAY)
                else:
                    print(f"\n❌ Failed to upload batch after {RETRY_ATTEMPTS} attempts: {e}")
                    if hasattr(e.response, 'text'):
                        print(f"   Response: {e.response.text}")
                    sys.exit(1)
    ...
    print(f"\n✅ Successfully uploaded {total_uploaded} points")
...
def verify_import(qdrant_url: str, collection_name: str, expected_count: int):
    ...
    try:
        ...
        if points_count == expected_count:
            print(f"✅ Import verified successfully - all {expected_count} points imported")
            return True
        else:
            print(f"⚠️  Warning: Expected {expected_count} points, but found {points_count}")
            return False
